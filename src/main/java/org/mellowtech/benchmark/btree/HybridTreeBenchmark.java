 ...
 */
public class HybridTreeBenchmark extends Benchmark {

  private BTree<String, CBString, byte[], CBByteArray> tree = null;
  private CBByteArray cbValue = new CBByteArray(value);

  public HybridTreeBenchmark(String[] keys, int keySize, int valueSize, Path benchmarkDir, Optional<PrintWriter> w,
                             Settings settings) {
    super("core.mellowtech.collections.imp.HybridTree",
  ...
  private void newTree(){
    try {
      ...
      BTreeBuilder builder = new BTreeBuilder().blobValues(settings.blobs()).memoryIndex(true).memoryMappedValues(settings.memMappedValues()).multiFileValues(true);
      Path p = testPath().resolve("hybridttreebenchmark");
